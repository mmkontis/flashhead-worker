...
# runpod's SDK invokes handlers from a running event loop, so the handler is
# async and stream sessions are awaited directly (asyncio.run would raise)
async def handler(job):
    i = job["input"]
    if i.get("mode") == "stream":
        return await handler_stream(job)
    return handler_batch(job)

async def handler_stream(job):
    ...
    stats = await run_avatar_session(i, pipeline)
